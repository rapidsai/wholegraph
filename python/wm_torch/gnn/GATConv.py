...
class gSpmmGAT(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x_neighboor: Tensor, csr_row_ptr: Tensor, csr_col_ind: Tensor,
                edge_weight: Tensor, sample_dup_count: Tensor):
        ctx.save_for_backward(x_neighboor, csr_row_ptr, csr_col_ind, edge_weight, sample_dup_count)
        out_tensor = torch.ops.wholegraph.gspmm_csr_weighted_forward(csr_row_ptr, csr_col_ind, x_neighboor, edge_weight)
        return out_tensor

    @staticmethod
    def backward(ctx, grad_outputs: Tensor):
        x_neighboor, csr_row_ptr, csr_col_ind, edge_weight, sample_dup_count = ctx.saved_tensors
        x_neighboor_grad = None
        edge_weight_grad = None
        in_grads = torch.ops.wholegraph.gspmm_csr_weighted_backward(csr_row_ptr, csr_col_ind, x_neighboor,
                                                                    edge_weight, sample_dup_count, grad_outputs)
...
class SpAddGAT(torch.autograd.Function):
    @staticmethod
    def forward(ctx, edge_weight_left: Tensor, edge_weight_right: Tensor, csr_row_ptr: Tensor, csr_col_ind: Tensor, sample_dup_count: Tensor):
        ctx.save_for_backward(csr_row_ptr, csr_col_ind, sample_dup_count)
        weight_graph_val_tensor = torch.ops.wholegraph.spadd_gat_csr_forward(csr_row_ptr, csr_col_ind, edge_weight_left, edge_weight_right)
        return weight_graph_val_tensor

    @staticmethod
    def backward(ctx, grad_outputs: Tensor):
        csr_row_ptr, csr_col_ind, sample_dup_count = ctx.saved_tensors
        grad_weight_left, grad_weight_right = torch.ops.wholegraph.spadd_gat_csr_backward(csr_row_ptr, csr_col_ind, sample_dup_count, grad_outputs)
...
class SpSoftMax(torch.autograd.Function):
    ...
    def forward(ctx, edge_weight: Tensor, csr_row_ptr: Tensor):
        edge_weight_softmax = torch.ops.wholegraph.edge_weight_softmax_forward(csr_row_ptr, edge_weight)
    ...
    def backward(ctx, grad_outputs: Tensor):
        ...
        grad_weight_softmax = torch.ops.wholegraph.edge_weight_softmax_backward(csr_row_ptr, edge_weight, grad_outputs)
...
class GATConv(torch.nn.Module):
    def __init__(self, in_channels: int,
        ...
        #left for target
        self.attn_l = torch.nn.Parameter(torch.FloatTensor(size=(1, num_heads, out_channels)))
        #right for neighbor
        self.attn_r = torch.nn.Parameter(torch.FloatTensor(size=(1, num_heads, out_channels)))
        self.leaky_relu = torch.nn.LeakyReLU(negative_slope)
        if bias:
            self.bias = torch.nn.Parameter(torch.zeros(size=(num_heads*out_channels, )))
        else:
            self.bias = None
    ...
    def forward(self, csr_row_ptr: Tensor, csr_col_ind: Tensor, sample_count: Tensor, x_neighboor: Tensor, x_target: Tensor):
        if self.add_self_loop:
            csr_row_ptr_looped, csr_col_ind_looped, sample_count_looped = torch.ops.wholegraph.csr_add_self_loop(
                csr_row_ptr, csr_col_ind, sample_count)
        else:
            csr_row_ptr_looped, csr_col_ind_looped, sample_count_looped = csr_row_ptr, csr_col_ind, sample_count
        ...
        feat_neighbor = h_neighbor.view(*neighbor_prefix_shape, self.num_heads, self.out_channels)
        ...
        csr_weight_graph_val = self.attention_compute(edge_weight_left, edge_weight_right, csr_row_ptr_looped, csr_col_ind_looped, sample_count_looped)
        ...
        #-> (TN, num_heads, d_out)
        y = self.aggregator(feat_neighbor, csr_row_ptr_looped, csr_col_ind_looped, edge_weight, sample_count_looped)

        if self.bias is not None:
            y = y + self.bias.view(*((1, ) * len(target_prefix_shape)), self.num_heads, self.out_channels)
