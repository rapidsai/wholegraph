...
class SpmmMean(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x_neighboor: Tensor, csr_row_ptr: Tensor, csr_col_ind: Tensor, sample_dup_count: Tensor):
        ctx.save_for_backward(csr_row_ptr, csr_col_ind, sample_dup_count, x_neighboor)
        mean_tensor = torch.ops.wholegraph.spmm_csr_noweight_forward(csr_row_ptr, csr_col_ind, x_neighboor, 1)
        return mean_tensor
...
class SpmmGCN(torch.autograd.Function):
    ...
    def forward(ctx, x_neighboor: Tensor, csr_row_ptr: Tensor, csr_col_ind: Tensor, sample_dup_count: Tensor):
        ...
        gcn_tensor = torch.ops.wholegraph.spmm_csr_noweight_forward(csr_row_ptr, csr_col_ind, x_neighboor, 2)
        return gcn_tensor
...
class SAGEConv(torch.nn.Module):
    def __init__(self,
        ...
        if aggregator == 'mean':
            ...
            self.lin = torch.nn.Linear(in_channels[0] + in_channels[1], out_channels, bias=bias)
    ...
    def forward(self, csr_row_ptr: Tensor, csr_col_ind: Tensor, sample_count: Tensor, x_neighboor: Tensor, x_target: Tensor):
        x_neighboor_agg = self.aggregator(x_neighboor, csr_row_ptr, csr_col_ind, sample_count)
        y = x_neighboor_agg
