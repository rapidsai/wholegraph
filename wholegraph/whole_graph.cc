...
struct WMSPHandle {
  bool is_host = false;
  CUdeviceptr whole_graph_cuptr;
  void* whole_graph_ptr = nullptr;
...
void WmspMalloc(void **ptr, size_t size, const int *dev_list, int dev_count) {
  ...
  WM_CU_CHECK(cuMemAddressReserve(&h->whole_graph_cuptr, h->total_aligned_memory_size, granularity, 0, 0));
  ...
  for (size_t i = 0; i < dev_list_vec.size(); i++) {
    ...
    WM_CU_CHECK(cuMemMap(h->whole_graph_cuptr + start_page * granularity, current_page_count * granularity, 0, h->handles[i], 0));
    ...
    WM_CU_CHECK(cuMemSetAccess(h->whole_graph_cuptr + start_page * granularity, current_page_count * granularity, &madesc[0], dev_list_vec.size()));
  }
  h->whole_graph_ptr = (void*) h->whole_graph_cuptr;
  *ptr = h->whole_graph_ptr;
...
void WmspMallocHost(void** ptr, size_t size) {
  ...
  WM_CUDA_CHECK(cudaHostGetDevicePointer(&h->whole_graph_ptr, h->host_ptr, 0));
  *ptr = h->whole_graph_ptr;
  wmsp_ptr_to_handle.emplace(*ptr, h);
}
static void WmspFreeLocked(void *ptr) {
  ...
  for (size_t i = 0; i < h->dev_list.size(); i++) {
    ...
    WM_CU_CHECK(cuMemUnmap(h->whole_graph_cuptr + start_page * h->granularity, page_count * h->granularity));
    WM_CU_CHECK(cuMemRelease(h->handles[i]));
  }
  WM_CU_CHECK(cuMemAddressFree(h->whole_graph_cuptr, h->total_aligned_memory_size));
  wmsp_ptr_to_handle.erase(ptr);
}
...
static void WmmpMallocHostLocked(void **ptr, size_t size, const int *ranks, int rank_count) {
  ...
  WM_CUDA_CHECK(cudaHostRegister(h->host_ptr, size, cudaHostRegisterDefault));
  WM_CUDA_CHECK(cudaHostGetDevicePointer(&h->whole_graph_ptr, h->host_ptr, 0));
  WM_CHECK(h->whole_graph_ptr == h->host_ptr);
  *ptr = h->whole_graph_ptr;
...
static void WmmpMallocLocked(void **ptr, size_t size, const int *ranks, int rank_count) {
  ...
  for (int i = 0; i < rank_count; i++) {
    ...
    if (mem_size > 0) {
      ...
      WM_CU_CHECK(cuMemMap(h->whole_graph_cuptr + h->memory_offsets[i], mem_size, 0, h->all_handles[i], 0));
  ...
  WM_CU_CHECK(cuMemSetAccess(h->whole_graph_cuptr, h->total_aligned_memory_size, &madesc, 1));
  h->whole_graph_ptr = (void*)h->whole_graph_cuptr;
  *ptr = h->whole_graph_ptr;
  wmmp_ptr_to_handle.emplace(*ptr, h);
...
static void WmmpFreeLocked(void *ptr) {
  ...
  for (size_t i = 0; i < h->ranks.size(); i++) {
    size_t mem_size = h->memory_offsets[i + 1] - h->memory_offsets[i];
    if (mem_size > 0) {
      WM_CU_CHECK(cuMemUnmap(h->whole_graph_cuptr + h->memory_offsets[i], mem_size));
