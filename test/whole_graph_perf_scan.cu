...
std::vector<double> AllGatherMultiProcess(double data, int rank, int size) {
  ...
  whole_graph::WmmpAllToAll(gvec.data(), sizeof(double), recv_vec.data(), sizeof(double));
  return recv_vec;
}
...
void MultiProcessFunc(int size) {
  MultiProcessRun(size, [](int rank, int size) {
    ...
    whole_graph::WmmpMalloc((void**)&memory_ptr, kMemorySize);
...
void ChunkedMultiProcessFunc(int size) {
  MultiProcessRun(size, [](int rank, int size) {
    ...
    whole_graph::WcmmpMalloc(&wcm, kMemorySize, 2 * 1024 * 1024);
