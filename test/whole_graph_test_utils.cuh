...
__global__ void GatherChunkedFloat(float* output, const whole_graph::WholeChunkedMemoryHandle* h, int* indice, size_t embedding_dim) {
  int bidx = blockIdx.x;
  int idx = indice[bidx];
...
__global__ void GatherChunkedFloat4(float* output, const whole_graph::WholeChunkedMemoryHandle* h, int* indice, size_t embedding_dim) {
  int bidx = blockIdx.x;
  int idx = indice[bidx];
  whole_graph::PtrGen<const whole_graph::WholeChunkedMemoryHandle, float> ptr_gen(h);
  float4* output_ptr = (float4*)(output + (size_t)bidx * embedding_dim);
