...
__global__ void CheckEdgeKernel(whole_graph::WholeChunkedMemoryHandle* csr_row_ptr_handle,
                                whole_graph::WholeChunkedMemoryHandle* csr_col_idx_handle,
  ...
  whole_graph::PtrGen<const whole_graph::WholeChunkedMemoryHandle, int64_t> csr_row_ptr_gen(csr_row_ptr_handle);
  whole_graph::PtrGen<const whole_graph::WholeChunkedMemoryHandle, int64_t> csr_col_idx_gen(csr_col_idx_handle);
...
void LoadGraphFunction(int pid, int pcount) {
  ...
  whole_graph::WmmpInit(pid, pcount, nullptr);
  ...
  whole_graph::WholeChunkedMemory_t csr_row_ptr, csr_col_idx;
  whole_graph::WcmmpMalloc(&csr_row_ptr, (node_count + 1) * sizeof(int64_t), sizeof(int64_t));
  whole_graph::WcmmpMalloc(&csr_col_idx, edge_count * sizeof(int64_t), sizeof(int64_t));

  int64_t final_edge_count;
  assert(cudaGetLastError() == cudaSuccess);
  whole_graph::WmmpLoadToChunkedCSRGraphFromEdgeBuffer(csr_row_ptr, csr_col_idx, local_edge_buffer, local_edge_count, node_count, edge_count, &final_edge_count, whole_graph::WMT_Int64, true, false, false);
  assert(cudaGetLastError() == cudaSuccess);
  printf("=>Rank=%d, final_edge_count=%ld\n", pid, final_edge_count);
  whole_graph::WmmpFreeEdgeData(local_edge_buffer, local_feature_buffer);

  whole_graph::WholeChunkedMemoryHandle* csr_row_ptr_handle = whole_graph::GetDeviceChunkedHandle(csr_row_ptr, dev_id);
  whole_graph::WholeChunkedMemoryHandle* csr_col_idx_handle = whole_graph::GetDeviceChunkedHandle(csr_col_idx, dev_id);
  ...
  whole_graph::WcmmpFree(csr_row_ptr);
  whole_graph::WcmmpFree(csr_col_idx);
