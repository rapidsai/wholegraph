...
void main_func(int rank, int size) {
  ...
  int *a, *b, *c;
  int *a_h, *b_h, *c_h;
  whole_memory::WmmpMalloc((void**)&a, kEltCount * sizeof(int));
  whole_memory::WmmpMalloc((void**)&b, kEltCount * sizeof(int), bv.data(), 2);
  whole_memory::WmmpMalloc((void**)&c, kEltCount * sizeof(int), cv.data(), 4);
  whole_memory::WmmpMallocHost((void**)&a_h, kEltCount * sizeof(int));
  whole_memory::WmmpMallocHost((void**)&b_h, kEltCount * sizeof(int), bv.data(), 2);
  whole_memory::WmmpMallocHost((void**)&c_h, kEltCount * sizeof(int), cv.data(), 4);
  ...
  whole_memory::WmmpFree(b);
  std::cerr << "After free b" << std::endl;
  whole_memory::WmmpFree(c);
  std::cerr << "After free c" << std::endl;
  whole_memory::WmmpFree(c_h);
  std::cerr << "After free c_h" << std::endl;
  whole_memory::WmmpFree(a_h);
