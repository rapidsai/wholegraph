...
__global__ void CopyEmbeddingFromMemoryBlockKernel(WMEmbType* wm_embedding,
  ...
  whole_memory::PtrGen<WMEmbType, EmbType> ptr_gen(wm_embedding, storage_offset);
...
__global__ void WholeMemoryGatherKernelOld(OutputT *__restrict__ output,
  ...
  whole_memory::PtrGen<const ParamHandleT, ParamT> ptr_gen(parameter, storage_offset);
