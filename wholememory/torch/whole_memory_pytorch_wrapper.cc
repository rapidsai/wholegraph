...
class WholeMemoryDeviceAllocator : public c10::Allocator {
 public:
  ...
  static c10::DataPtr allocate_with_ranks(size_t nbytes, const std::vector<int>& ranks) {
    void* ptr = nullptr;
    whole_memory::WmmpMalloc(&ptr, nbytes, ranks.data(), ranks.size());
...
class WholeMemoryUnifiedAllocator : public c10::Allocator {
 public:
  ...
  static c10::DataPtr allocate_with_ranks(size_t nbytes, const std::vector<int>& ranks) {
    void* ptr = nullptr;
    whole_memory::WmmpMallocHost(&ptr, nbytes, ranks.data(), ranks.size());
    c10::Device device = GetCurrentCUDADevice();
    //(void* data, void* ctx, DeleterFnPtr ctx_deleter, Device device)
    return {ptr, ptr, &DeleteWholeMemoryMemory, device};
  }
...
torch::Tensor WholeMemoryTensorViewFromDevice(torch::Tensor t, torch::Device dev) {
  ...
  assert(whole_memory::WmmpCanAccessFrom(storage_impl_ptr->data(), dev_idx));
...
py::tuple WholeMemoryAggregateSize(size_t local_size, const std::vector<int> &ranks) {
  ...
  total_size = whole_memory::WmmpAggregateSize(local_size, &offset, ranks.data(), ranks.size());
  return py::make_tuple(total_size, offset);
}

void WholeMemoryBarrier(std::vector<int>& ranks) {
  whole_memory::WmmpBarrier(ranks.data(), ranks.size());
}

whole_memory::pytorch::ChunkedTensor WholeMemoryCreateChunkedTensorScalarType(const std::vector<int64_t> &sizes,
                                                                    const std::vector<int64_t> &strides,
                                                                    torch::ScalarType type,
                                                                    const std::vector<int> &ranks) {
  size_t elt_size = c10::elementSize(type);
  ...
  c10::intrusive_ptr<whole_memory::pytorch::StorageImpl>
      storage_impl_ptr = c10::make_intrusive<whole_memory::pytorch::StorageImpl>(storage_elt_count * elt_size,
                                                                                 min_granularity,
                                                                                 ranks);
  whole_memory::pytorch::Storage storage(storage_impl_ptr);
  caffe2::TypeMeta data_type;
  data_type = type;
  whole_memory::pytorch::ChunkedTensor t = whole_memory::pytorch::ChunkedTensor::wrap_tensor_impl(
      c10::intrusive_ptr<whole_memory::pytorch::ChunkedTensorImpl>::make(std::move(storage), data_type));
  t.getIntrusivePtr()->set_sizes_and_strides(c10::IntArrayRef({sizes}), c10::IntArrayRef({fixed_strides}));
  return t;
}

whole_memory::pytorch::ChunkedTensor WholeMemoryCreateChunkedTensor(const std::vector<int64_t> &sizes,
                                                                    const std::vector<int64_t> &strides,
                                                                    py::object dtype,
                                                                    const std::vector<int> &ranks) {
  torch::ScalarType type = torch::python::detail::py_object_to_dtype(std::move(dtype));
...
whole_memory::pytorch::ChunkedTensor WholeMemoryGetSubChunkedTensor(const whole_memory::pytorch::ChunkedTensor& ct,
  ...
  whole_memory::pytorch::Storage storage = ct.storage();
  whole_memory::pytorch::ChunkedTensor new_t = whole_memory::pytorch::ChunkedTensor::wrap_tensor_impl(
      c10::intrusive_ptr<whole_memory::pytorch::ChunkedTensorImpl>::make(std::move(storage), ct.dtype()));
...
void WholeMemoryLoadEmbeddingToTensorFromFilePrefix(torch::Tensor t,
  ...
  whole_memory::WmmpLoadEmbeddingFromFilelists(wm_embedding,
                                               storage_offset,
                                               table_size,
                                               embedding_dim,
                                               embedding_stride,
                                               file_prefix,
                                               whole_memory::pytorch::C10ScalarToWMType(src_type),
                                               whole_memory::pytorch::C10ScalarToWMType(emb_type),
                                               stream);
}
void WholeMemoryLoadEmbeddingToChunkedTensorFromFilePrefix(const whole_memory::pytorch::ChunkedTensor& ct,
                                                           py::object src_dtype,
                                                           py::object emb_dtype,
                                                           int64_t table_size,
                                                           int64_t embedding_dim,
  ...
  torch::ScalarType src_scalar_type = torch::python::detail::py_object_to_dtype(std::move(src_dtype));
  torch::ScalarType emb_scalar_type = torch::python::detail::py_object_to_dtype(std::move(emb_dtype));
  whole_memory::WholeChunkedMemory_t wmt_embedding = ct.GetChunkedMemory();
  ...
  emb_type = emb_scalar_type;
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  whole_memory::WmmpLoadChunkedEmbeddingFromFilelists(wmt_embedding,
                                                      storage_offset,
                                                      table_size,
                                                      embedding_dim,
                                                      embedding_stride,
...
std::vector<int64_t> WholeMemoryLoadEdgeIndexFromFilePrefix(const std::string &file_prefix,
  ...
  int64_t local_edge_count = whole_memory::WmmpTraverseLoadEdgeDataFromFileList(&local_edge_buffer,
                                                                                ...
                                                                                &dst_node_count,
                                                                                file_prefix,
                                                                                whole_memory::pytorch::C10ScalarToWMType(id_type),
                                                                                edge_embedding_dim);
...
std::vector<torch::Tensor> WholeMemoryAllocateCSRGraph(const std::vector<int64_t> &edge_index,
  ...
  int64_t est_edge_count = whole_memory::EstimateEdgeCount(total_src_node_count, edge_index[3], directed, add_self_loop);
...
std::vector<whole_memory::pytorch::ChunkedTensor> WholeMemoryAllocateChunkedCSRGraph(const std::vector<int64_t> &edge_index,
                                                                                     py::object id_dtype,
                                                                                     bool directed,
                                                                                     bool reverse_edge,
                                                                                     bool add_self_loop) {
  ...
  int64_t
      est_edge_count = whole_memory::EstimateEdgeCount(total_src_node_count, edge_index[3], directed, add_self_loop);
...
int64_t WholeMemoryLoadToCSRGraphFromEdgeBuffer(const std::vector<int64_t> &edge_index,
  ...
  whole_memory::WmmpLoadToCSRGraphFromEdgeBuffer(wm_csr_row_ptr.data_ptr(),
...
int64_t WholeMemoryLoadToChunkedCSRGraphFromEdgeBuffer(const std::vector<int64_t> &edge_index,
                                                       const whole_memory::pytorch::ChunkedTensor& wm_csr_row_ptr,
                                                       const whole_memory::pytorch::ChunkedTensor& wm_csr_col_idx,
                                                       py::object id_dtype,
                                                       bool directed,
                                                       bool reverse_edge,
                                                       bool add_self_loop) {
  ...
  whole_memory::WmmpLoadToChunkedCSRGraphFromEdgeBuffer(wmt_csr_row_ptr,
                                                        wmt_csr_col_idx,
                                                        (void *) edge_index[1],
                                                        edge_index[0],
                                                        edge_index[4],
...
void WholeMemoryFreeEdgeIndex(const std::vector<int64_t>& edge_index) {
  ...
  whole_memory::WmmpFreeEdgeData((void*)edge_index[1], (void*)edge_index[2]);
}
...
PYBIND11_MODULE(wholememory_pytorch, m) {
  py::class_<whole_memory::pytorch::ChunkedTensor>(m, "ChunkedTensor")
      .def(py::init<const whole_memory::pytorch::ChunkedTensor&>())
      .def("get_ptr", &whole_memory::pytorch::ChunkedTensor::get_ptr)
      .def_property_readonly("shape", &whole_memory::pytorch::ChunkedTensor::sizes)
      .def_property_readonly("stride", &whole_memory::pytorch::ChunkedTensor::strides)
      .def_property_readonly("dtype", [](const whole_memory::pytorch::ChunkedTensor& ct) {
