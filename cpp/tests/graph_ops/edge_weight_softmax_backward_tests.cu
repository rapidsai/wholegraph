...
typedef struct EdgeWeightSoftmaxBackwardTestParam {
  EdgeWeightSoftmaxBackwardTestParam& set_graph_row_num(int new_graph_row_num)
  {
    graph_row_num = new_graph_row_num;
    return *this;
  }
  EdgeWeightSoftmaxBackwardTestParam& set_graph_col_num(int new_graph_col_num)
  {
    graph_col_num = new_graph_col_num;
    return *this;
  }
  EdgeWeightSoftmaxBackwardTestParam& set_graph_edge_num(int new_graph_edge_num)
  {
    graph_edge_num = new_graph_edge_num;
    return *this;
  }
  EdgeWeightSoftmaxBackwardTestParam& set_num_heads(int new_num_heads)
  {
    num_heads = new_num_heads;
    return *this;
  }
  EdgeWeightSoftmaxBackwardTestParam& set_weight_dtype(wholememory_dtype_t new_weight_dtype)
  {
...
} EdgeWeightSoftmaxBackwardTestParam;

class EdgeWeightSoftmaxBackwardParameterTests
  : public ::testing::TestWithParam<EdgeWeightSoftmaxBackwardTestParam> {};

TEST_P(EdgeWeightSoftmaxBackwardParameterTests, EdgeWeightSoftmaxBackwardParameterTest)
{
  ...
  graph_ops::testing::gen_features(host_grad_weight_softmax_ptr, weight_matrix_desc);

  void* host_output_grad_weight_ptr =
    (void*)malloc(wholememory_get_memory_size_from_matrix(&weight_matrix_desc));
  void* host_ref_output_grad_weight_ptr =
    (void*)malloc(wholememory_get_memory_size_from_matrix(&weight_matrix_desc));
  void *dev_csr_row_ptr, *dev_weight_ptr, *dev_grad_weight_softmax_ptr, *dev_output_grad_weight_ptr;
  EXPECT_EQ(
  ...
  EXPECT_EQ(
    ...
    cudaSuccess);
  EXPECT_EQ(cudaMalloc(&dev_grad_weight_softmax_ptr,
                       wholememory_get_memory_size_from_matrix(&weight_matrix_desc)),
            cudaSuccess);
  EXPECT_EQ(cudaMalloc(&dev_output_grad_weight_ptr,
                       wholememory_get_memory_size_from_matrix(&weight_matrix_desc)),
            cudaSuccess);
  ...
  EXPECT_EQ(cudaMemcpy(dev_weight_ptr,
                       ...
                       cudaMemcpyHostToDevice),
            cudaSuccess);
  EXPECT_EQ(cudaMemcpy(dev_grad_weight_softmax_ptr,
  ...
  wholememory_tensor_description_t csr_row_ptr_tensor_desc, weight_tensor_desc,
    grad_weight_softmax_tensor_desc, output_grad_weight_tensor_desc;
  wholememory_tensor_t csr_row_ptr_tensor, weight_tensor, grad_weight_softmax_tensor,
  ...
  wholememory_copy_matrix_desc_to_tensor(&grad_weight_softmax_tensor_desc, &weight_matrix_desc);
  wholememory_copy_matrix_desc_to_tensor(&output_grad_weight_tensor_desc, &weight_matrix_desc);
  ...
  EXPECT_EQ(
    wholememory_make_tensor_from_pointer(
      &grad_weight_softmax_tensor, dev_grad_weight_softmax_ptr, &grad_weight_softmax_tensor_desc),
    WHOLEMEMORY_SUCCESS);
  EXPECT_EQ(wholememory_make_tensor_from_pointer(
  ...
  graph_ops::testing::host_edge_weight_softmax_backward(host_csr_row_ptr,
                                                        csr_row_ptr_array_desc,
                                                        host_weight_ptr,
                                                        weight_matrix_desc,
                                                        host_grad_weight_softmax_ptr,
                                                        ...
                                                        weight_matrix_desc);
  graph_ops::testing::host_check_float_matrix_same(host_output_grad_weight_ptr,
                                                   weight_matrix_desc,
                                                   host_ref_output_grad_weight_ptr,
                                                   weight_matrix_desc);
  ...
  EXPECT_EQ(cudaFree(dev_grad_weight_softmax_ptr), cudaSuccess);
  EXPECT_EQ(cudaFree(dev_output_grad_weight_ptr), cudaSuccess);
  if (host_csr_row_ptr != nullptr) free(host_csr_row_ptr);
  if (host_weight_ptr != nullptr) free(host_weight_ptr);
  if (host_grad_weight_softmax_ptr != nullptr) free(host_grad_weight_softmax_ptr);
  if (host_output_grad_weight_ptr != nullptr) free(host_output_grad_weight_ptr);
  if (host_ref_output_grad_weight_ptr != nullptr) free(host_ref_output_grad_weight_ptr);
...
INSTANTIATE_TEST_SUITE_P(EdgeWeightSoftmaxBackwardTests,
                         EdgeWeightSoftmaxBackwardParameterTests,
                         ::testing::Values(EdgeWeightSoftmaxBackwardTestParam()
                                             .set_graph_row_num(3)
                                             .set_graph_col_num(5)
                                             .set_graph_edge_num(7)
                                             .set_num_heads(2)));
