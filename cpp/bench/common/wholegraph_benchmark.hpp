...
struct Metric {
  Metric(const std::string& metrics_name, const std::string& metrics_unit, const double metrics_value, bool inv) {
    name = metrics_name;
    unit = metrics_unit;
    value = metrics_value;
    invert = inv;
...
void SingleProcessMeasurePerformance(std::function<void()> cuda_fn,
                        ...
                        std::function<void(cudaError_t)> check_fn = [](cudaError_t e){WHOLEMEMORY_CHECK_NOTHROW(e == cudaSuccess);});
